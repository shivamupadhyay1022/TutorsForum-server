...
export async function register(req, res) {
  try {
    ...
    const emcheck = await emailExist();
    const uscheck = await userExist();
    if (emcheck && uscheck) {
      ...
      res.status(400).json({ msg: "Falied due to non unique credentials." });
  } catch (err) {
    console.log(err);
    res.status(500).send({ error:"error2" + err });
  }
}
        ...
        //   };
         // const uscheck = await userExist();
          if (query) {
            ...
            const { password, ...rest } = Object.assign({}, query.toJSON());
  ...
  try {
      ...
      // if (authHeader.startsWith("Bearer")){
        if(userId){
          ...
          // })
          // update the data
          UserModel.updateOne({ _id :userId  },body).then((body)=>{
            return res.status(201).send({ msg : "Record Updated...!"});
          }).catch((err)=>{
              ...
              .then(user => {
                  bcrypt.hash(password, 10)
                      .then(hashedPassword => {
                          ...
                          { password: hashedPassword}).then((password)=>{
...
export async function resetPassword2(req,res){
  // if(!req.app.locals.resetSession) 
  // return res.status(440).send({error : "Session expired!"});
...
/** GET: http://localhost:8080/api/user/example123 */
export async function getUser2(req,res){
      ...
      //   };
       // const uscheck = await userExist();
        if (query) {
          ...
          /** remove password from user */
          // mongoose return unnecessary data with object so convert it into json
          const rest  = Object.assign({}, query.toJSON());
